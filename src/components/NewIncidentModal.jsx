...
export default function NewIncidentModal({
  ...
  return (
    <div 
      ...
      aria-modal="true"
    >
      <div className="relative w-full max-w-lg bg-elevated border border-line-strong rounded-2xl shadow-2xl overflow-hidden my-6">
        ...
        <div className="flex items-center justify-between p-4 sm:p-5 border-b border-line bg-app-2/60">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-bold text-ink">Log Critical Incident</h2>
              <p className="text-xs text-ink-3 font-mono">Dispatcher Priority Transmission</p>
            </div>
          </div>

          <button
            onClick={onClose}className="p-1.5 rounded-lg text-ink-3 hover:text-ink hover:bg-app-2 transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4 text-xs font-sans">
          ...
          <div>
            <label className="block text-ink-2 font-bold uppercase tracking-wider mb-1 font-mono text-[11px]">
              Incident Title
            </label>
            <input
              type="text"
              ...
              className="w-full bg-app-2 text-ink p-2.5 rounded-lg border border-line focus:outline-none focus:border-rose-500"
            />
          </div>
          ...
          <div className="pt-2 flex items-center justify-end gap-3 border-t border-line">
            <button
              type="button"
              onClick={onClose}className="px-4 py-2 rounded-xl text-ink-3 hover:text-ink">
              Cancel
            </button>
